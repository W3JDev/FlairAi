...
function ControlTray({ children }: ControlTrayProps) {
  ...
  useEffect(() => {
    if (!connected && connectButtonRef.current) {
      // connectButtonRef.current.focus(); // Auto-focus can be disruptive on mobile
    }
  }, [connected]);
  ...
  useEffect(() => {
    ...
    if (connected && !muted && audioRecorder) {
      // Use type assertion for audioRecorder.on
      (audioRecorder as EventEmitter<any>).on('data', onData);
      audioRecorder.start();
    } else {
      audioRecorder.stop();
    }
    return () => {
      // Use type assertion for audioRecorder.off
      (audioRecorder as EventEmitter<any>).off('data', onData);
    };
      ...
      <div className={cn('connection-container', { connected })}>
        <div className="connection-button-container">
          <button
            ...
            aria-label={connected ? "Disconnect session" : "Connect session"}
          >
